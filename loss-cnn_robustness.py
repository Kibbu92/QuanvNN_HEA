...
if __name__ == '__main__':
    ...
    blurred_images_X_train = np.array([apply_gaussian_blur(image) for image in X_TRAIN])
    ...
    plt.subplot(1, 2, 2)
    plt.imshow(noisy_images_X_train[0], cmap='gray')
    ...
    # Modified image on the right
    plt.subplot(1, 2, 2)
    plt.imshow(images_with_black_rectangles_X_train[0], cmap='gray')
    plt.title('Noisy Image')
    ...
    # Modified image on the right
    plt.subplot(1, 2, 2)
    plt.imshow(blurred_images_X_train[0], cmap='gray')
    plt.title('Noisy Image')
    ...
    for j in range(len(KNL)):
        ...
        for h in range(len(OPT)):
            ...
            SAVE_PATH = os.path.join('results', DATSET_NAME + f'_CNN_Robust_{dataset_type}',  f'{KERNEL_SIZE[0]*KERNEL_SIZE[1]}_{KERNEL_SIZE}' , f'{OPT[h]}_{KERNEL_SIZE[0]*KERNEL_SIZE[1]}_{KERNEL_SIZE}')
